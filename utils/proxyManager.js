...
function flagProxy(proxy, reason, account = null) {
  ...
  if (existing) {
    ...
    existing.reasons.push({ reason, account, timestamp: new Date().toISOString() });
  } else {
...
function unflagProxy(proxy) {
  ...
  flaggedData.proxies = flaggedData.proxies.filter(fp => fp.proxyId !== proxyId);
  saveFlaggedProxies(flaggedData);
...
function getActiveProxy(username, accountProxies) {
  ...
  console.warn(`⚠️  All proxies for ${username} are flagged! Using first proxy anyway.`);
