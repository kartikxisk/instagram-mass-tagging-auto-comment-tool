...
class AutomationRunner extends EventEmitter {
  ...
  async start(options = {}) {
    ...
    try {
      ...
      for (let i = 0; i < config.accounts.length; i++) {
        if (this.shouldStop) {
          this.emit('log', { type: 'warning', message: '⏹️ Stopping automation...' });
          break;
        }
        ...
        await this.processAccount(account, allTags, config, i, options, utils);
        ...
        if (i < config.accounts.length - 1 && !this.shouldStop) {
          const delayTime = Math.floor(Math.random() * 15000) + 5000; // 5-20 seconds
          this.emit('log', { type: 'info', message: `⏳ Waiting ${Math.round(delayTime/1000)}s before next account...` });
  ...
  async processAccount(account, allTags, config, accountIndex, options, utils) {
    ...
    this.emit('log', { type: 'info', message: `👤 Processing account: ${account.username}` });
    
    if (proxy) {
      this.emit('log', { type: 'info', message: `🌐 Using proxy: ${proxy.address}:${proxy.port}` });
    } else {
      this.emit('log', { type: 'warning', message: `⚠️ No proxy assigned for ${account.username}` });
    }
    ...
    try {
      ...
      this.emit('log', { type: 'info', message: `🔐 Checking session for ${account.username}...` });
      const loginResult = await this.loginWithLogs(page, account, utils);

      if (!loginResult.success) {
        this.emit('log', { type: 'error', message: `Login failed for ${account.username}: ${loginResult.error}` });
        // Log to file as well
      ...
      this.emit('log', { type: 'success', message: `✅ Logged in as ${account.username}` });

      // Navigate to target post
      this.emit('log', { type: 'info', message: '📍 Navigating to target post...' });
      await page.goto(config.targetPost, { waitUntil: 'networkidle2', timeout: 60000 });

      // Check if stopped
      if (this.shouldStop) {
        this.emit('log', { type: 'warning', message: '⏹️ Stopping after navigation...' });
        return;
      }

      // Get only untagged users for this account
      ...
      if (untaggedForAccount.length === 0) {
        this.emit('log', { type: 'warning', message: `⚠️ No more untagged users available for ${account.username}` });
        return;
      }
      
      this.emit('log', { type: 'info', message: `📋 Got ${untaggedForAccount.length} untagged users for this account` });
      ...
      this.emit('log', { type: 'info', message: `📝 Planning to post ${commentsToPost} comments (${tagsPerCommentMin}-${tagsPerCommentMax} tags each)` });

      // Post comments
      for (let i = 0; i < commentsToPost; i++) {
        if (this.shouldStop) {
          this.emit('log', { type: 'warning', message: '⏹️ Stopping comment loop...' });
        ...
        // Format tags with @ for display
        const tagsDisplay = tags.map(t => `@${t}`).join(' ');
        this.emit('log', { type: 'info', message: `💬 Comment ${i + 1}/${commentsToPost} (${tags.length} tags): ${tagsDisplay}` });
        ...
        if (result.success) {
          ...
          this.emit('log', { type: 'success', message: `✅ Comment ${i + 1} posted successfully` });
          this.stats.commentsPosted++;
          this.stats.successfulComments++;
          this.stats.tagsPosted += tags.length;
          
          // Log remaining untagged count
          const remaining = utils.tagTracker.getStats();
          this.emit('log', { type: 'info', message: `📊 Total tagged: ${remaining.totalTagged} users` });
          ...
          this.emit('log', { type: 'error', message: `❌ Comment ${i + 1} failed: ${result.error}` });
          ...
          if (result.error.includes('blocked') || result.error.includes("Couldn't post")) {
            this.emit('log', { type: 'warning', message: '🚫 Action blocked detected, skipping account' });
        ...
        if (i < commentsToPost - 1 && !this.shouldStop) {
          ...
          this.emit('log', { type: 'info', message: `⏳ Waiting ${Math.round(delayTime/1000)}s before next comment...` });
          await this.sleep(delayTime);
        }
      }
      ...
      if (!this.shouldStop) {
        this.emit('log', { type: 'info', message: `🍪 Saving session for ${account.username}` });
        await utils.sessionManager.saveCookies(page, account.username);
        this.emit('log', { type: 'success', message: `✅ Finished processing ${account.username}` });
      }

    } catch (error) {
      // Only log error if not due to stop
      if (!this.shouldStop) {
        this.emit('log', { type: 'error', message: `🚫 Error processing ${account.username}: ${error.message}` });
  ...
  async postComment(page, tags, utils) {
    try {
      ...
      // Type each tag
      for (let tag of tags) {
        // Ensure tag doesn't start with @ (we add it ourselves)
        if (tag.startsWith('@')) {
          tag = tag.substring(1);
        }
        
        await page.keyboard.type('@', { delay: utils.delay.getTypingDelay() });
        await utils.humanBehavior.randomWait(200, 500);
      ...
      // Submit
      await utils.humanBehavior.humanPause(0.5, 1.5);
      await page.keyboard.press('Enter');
      ...
      if (errorDetected) {
        return { success: false, error: "Couldn't post comment - Instagram blocked this action" };
      }
  ...
  getProxyForAccount(account, globalProxies, accountIndex) {
    if (account.proxies && account.proxies.length > 0) {
      return account.proxies[accountIndex % account.proxies.length];
    }
    if (account.proxy && account.proxy.address) {
      return account.proxy;
    }
    if (globalProxies.length > 0) {
      return globalProxies[accountIndex % globalProxies.length];
    }
    return null;
  }
